...
const makeMessagesSocket = (config) => {
    ...
    const kev = new Z(Utils_1, waUploadToServer, relayMessage, config, sock);
    ...
    return {
        ...
        sendStatusMention: async (content, jids = []) => {
            return await kev.sendStatusWhatsApp(content, jids);
        },
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = kev.detectType(content);
            if (typeof content === 'object' && 'disappearingMessagesInChat' in content &&
                ...
                if (messageType) {
                    switch (messageType) {
                        case 'PAYMENT':
                            const paymentContent = await kev.handlePayment(content, quoted);
                            return await relayMessage(jid, paymentContent, {
                                messageId: Utils_1.generateMessageID(),
                                ...getParticipantAttr()
                            });
                        case 'PRODUCT':
                            const productContent = await kev.handleProduct(content, jid, quoted);
                        ...
                        case 'INTERACTIVE':
                            const interactiveContent = await kev.handleInteractive(content, jid, quoted);
                        ...
                        case 'INTERACTIVE_BUTTONS':
                            const ibContent = await kev.handleInteractiveButtons(content, jid, quoted);
                        ...
                        case 'ALBUM':
                            return await kev.handleAlbum(content, jid, quoted)
                        case 'EVENT':
                            return await kev.handleEvent(content, jid, quoted)
                        case 'POLL_RESULT':
                            return await kev.handlePollResult(content, jid, quoted)
                        case 'GROUP_STORY':
                            return await kev.handleGroupStory(content, jid, quoted)
                    }
                }
